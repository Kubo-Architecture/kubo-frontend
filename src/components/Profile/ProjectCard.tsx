...
const ProjectCard = ({ project }: any) => {
  ...
  const handleImageError = (event: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    // Evita loop infinito caso o ícone também falhe
    if (!target.src.includes('kubo-main-icon')) {
      target.src = KuboIcon;
      target.className =
        'w-full h-full p-10 object-contain bg-gray-100 dark:bg-[#202830]';
    }
  };

  return (
    <div
      className="bg-white dark:bg-[#151B23] min-h-[22rem] rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer border border-gray-200 dark:border-[#3d444d]"
      onClick={handleClick}
    >
      {project.photo_url ? (
      ...
      <div className="p-5">
        ...
        <div className="flex items-center text-gray-600 dark:text-neutral-400 mb-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-4 w-4 mr-1 text-gray-400 dark:text-neutral-500"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
